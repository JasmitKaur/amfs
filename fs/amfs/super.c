...
extern void amfs_destroy_h_list(void);
extern int amfs_update_pattern_db(struct amfs_sb_info *spd);
...
static void amfs_put_super(struct super_block *sb)
{
	...
	err = amfs_update_pattern_db(spd);
	if (err) {
		pr_err("amfs_put_super: amfs_update_pattern_db failed!"
		"can not umnount amfs else malware db will be lost!\n");
		goto out;
	}
