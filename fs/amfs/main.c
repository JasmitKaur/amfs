...
extern int amfs_init_h_list(struct amfs_sb_info *spd);
extern char *amfs_is_mount_option_valid(char *lower_path, char *raw_data);
...
static int amfs_read_super(struct super_block *sb, void *raw_data, int silent)
{
	...
	if (!dev_name) {
		printk(KERN_ERR
		       "amfs_read_super: missing dev_name argument\n");
		goto out;
	}
	...
	if (!AMFS_SB(sb)) {
		printk(KERN_CRIT "amfs_read_super: out of memory\n");
		err = -ENOMEM;
		goto out_free;
	}
...
{
	int err = -EINVAL;
	struct dentry *d_mounted = NULL;
	void *lower_path_name = (void *) dev_name;
	struct amfs_sb_info *spd;
	char *file_path = NULL;

	file_path = amfs_is_mount_option_valid(lower_path_name,
						(char *) raw_data);
	err = (int) PTR_ERR(file_path);
	if (!file_path || IS_ERR(file_path) || !strlen(file_path)) {
		pr_err("amfs_mount: pattern db filepath should be "
			"given as per specification! %d\n", err);
		goto out_err;
	}

	d_mounted = mount_nodev(fs_type, flags, lower_path_name,
			   amfs_read_super);

	if (d_mounted) {
		spd = AMFS_SB(d_mounted->d_sb);
		if (!spd) {
			pr_err("amfs_mount: Invalid sb info!\n");
			goto out_err;
		}
		spd->patt_db_path = file_path;

		err = amfs_init_h_list(spd);
		if (err) {
			pr_err("amfs_mount: amfs_init_h_list failed! "
							"err(%d)\n", err);
			goto out_release;
		}
	}
...
out_release:
	...
	atomic_dec(&(amfs_lower_super(d_mounted->d_sb))->s_active);
	amfs_set_lower_super(d_mounted->d_sb, NULL);
	kfree(spd->patt_db_path);
	kfree(spd);
	d_mounted->d_sb->s_fs_info = NULL;
	iput(d_mounted->d_inode);
	dput(d_mounted);
out_err:
	return ERR_PTR(err);
}
...
MODULE_AUTHOR("Jasmit Kaur Saluja, under the guidance of Prof. Erez Zadok,"
		" Filesystems and Storage Lab, Stony Brook University,"
